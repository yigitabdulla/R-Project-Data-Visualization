#I'm writing this area so I can run it on my own computer
getwd()
setwd("C:/Users/Aidata/Desktop/")
getwd()
data<- read.csv('data.csv', header=TRUE, sep=";",dec=".")
View(data)


find_cross_product <- function(data, variables, factors) {
  for (variable in variables) {
    data[[variable]] <- gsub(",", ".", data[[variable]])
    data[[variable]] <- as.numeric(data[[variable]])
  }
  
  my_length <- function(x) {
    count <- 0
    for (elem in x) {
      count <- count + 1
    }
    count
  }
  
  my_unique <- function(x) {
    unique_values <- vector()
    for (elem in x) {
      if (!(elem %in% unique_values)) {
        unique_values <- c(unique_values, elem)
      }
    }
    unique_values
  }
  
  my_sum <- function(x) {
    total <- 0
    for (element in x) {
      if (!is.na(element)) {
        total <- total + element
      }
    }
    total
  }
  
  cross_product_values <- matrix(
    NA, 
    nrow = my_length(my_unique(do.call("paste", data[, factors, drop = FALSE]))),
    ncol = 1
  )
  
  colnames(cross_product_values) <- "CrossProduct"
  
  unique_factors <- my_unique(do.call("paste", data[, factors, drop = FALSE]))
  
  for (i in 1:my_length(unique_factors)) {
    subset_data <- data[do.call("paste", data[, factors, drop = FALSE]) == unique_factors[i], variables]
    cross_product_values[i, 1] <- my_sum(subset_data[, 1] * subset_data[, 2])
  }
  
  result <- data.frame(Factors = unique_factors, CrossProduct = cross_product_values)
  return(result)
}

cross_Group <- find_cross_product(data, c("Var1", "Var2"), c("Group"))
print(cross_Group)

cross_Gender <- find_cross_product(data, c("Var1", "Var2"), c("Gender"))
print(cross_Gender)

cross_Both <- find_cross_product(data, c("Var1", "Var2"), c("Gender", "Group"))
print(cross_Both)






